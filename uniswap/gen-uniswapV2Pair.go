// Generated using the ABI from:
// https://unpkg.com/@uniswap/v2-core@1.0.0/build/IUniswapV2Pair.json
//
// Generated using the celo-blockchain abigen tool:
// abigen --abi <path-to-abi> --pkg uniswap --type UniswapV2Pair --out gen-uniswapV2Pair.go
...
// ParseUniswapV2PairABI parses the ABI
func ParseUniswapV2PairABI() (*abi.ABI, error) {
	parsed, err := abi.JSON(strings.NewReader(UniswapV2PairABI))
	if err != nil {
		return nil, err
	}
...
func (_UniswapV2Pair *UniswapV2PairFilterer) TryParseLog(log types.Log) (eventName string, event interface{}, ok bool, err error) {
	eventName, ok, err = _UniswapV2Pair.contract.LogEventName(log)
	if err != nil || !ok {
	...
	switch eventName {
	case "Approval":
		event, err = _UniswapV2Pair.ParseApproval(log)
	case "Burn":
		event, err = _UniswapV2Pair.ParseBurn(log)
	case "Mint":
		event, err = _UniswapV2Pair.ParseMint(log)
	case "Swap":
		event, err = _UniswapV2Pair.ParseSwap(log)
	case "Sync":
		event, err = _UniswapV2Pair.ParseSync(log)
	case "Transfer":
		event, err = _UniswapV2Pair.ParseTransfer(log)
	}
	if err != nil {
		return "", nil, false, err
	}
