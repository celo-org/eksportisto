...
// abigen --abi <path-to-abi> --pkg uniswap --type UniswapV2Factory --out gen-uniswapV2Factory.go
...
// ParseUniswapV2FactoryABI parses the ABI
func ParseUniswapV2FactoryABI() (*abi.ABI, error) {
	parsed, err := abi.JSON(strings.NewReader(UniswapV2FactoryABI))
	if err != nil {
		return nil, err
	}
...
func (_UniswapV2Factory *UniswapV2FactoryFilterer) TryParseLog(log types.Log) (eventName string, event interface{}, ok bool, err error) {
	eventName, ok, err = _UniswapV2Factory.contract.LogEventName(log)
	if err != nil || !ok {
	...
	switch eventName {
	case "PairCreated":
		event, err = _UniswapV2Factory.ParsePairCreated(log)
	}
	if err != nil {
		return "", nil, false, err
	}
